...
function createTextNode(txt, x, y, size=12, color={r:0.91,g:0.91,b:0.96}){
  const t = figma.createText();
  t.characters = txt; t.x = x; t.y = y; t.fontSize = size;
  t.fills = [{type:'SOLID', color}];
  return t;
}

function createRoundedRect(x,y,w,h, radius=12, fill={r:0.07,g:0.09,b:0.22}, stroke={r:0.16,g:0.18,b:0.33}){
...
function createTabComponents(){
  const container = figma.createFrame();
  container.name = 'UI Components';
  container.layoutMode = 'HORIZONTAL'; container.itemSpacing = 24; container.counterAxisSizingMode = 'AUTO'; container.primaryAxisSizingMode = 'AUTO';
  container.fills = [];
  // inactive
  const c0 = figma.createComponent(); c0.name = 'Tab/active=false'; c0.resize(88,32);
  const r0 = createRoundedRect(0,0,88,32,16, {r:0.07,g:0.09,b:0.22}, {r:0.16,g:0.18,b:0.33});
  const t0 = createTextNode('Инсайт', 16, 8, 12, {r:0.66,g:0.65,b:0.78});
  c0.appendChild(r0); c0.appendChild(t0);
  // active
  const c1 = figma.createComponent(); c1.name = 'Tab/active=true'; c1.resize(88,32);
  const r1 = createRoundedRect(0,0,88,32,16, {r:0.43,g:0.36,b:0.94}, null);
  const t1 = createTextNode('Инсайт', 16, 8, 12, {r:1,g:1,b:1});
  c1.appendChild(r1); c1.appendChild(t1);
  // interactive reactions
  c0.reactions = [{trigger:{type:'ON_CLICK'}, action:{type:'CHANGE_TO', destinationId: c1.id}}];
  c1.reactions = [{trigger:{type:'ON_CLICK'}, action:{type:'CHANGE_TO', destinationId: c0.id}}];
  container.appendChild(c0); container.appendChild(c1);
  const set = figma.combineAsVariants([c0,c1], container);
  set.name = 'Tab'; set.variantGroupProperties = {active:{values:['true','false']}};
  return {container, set};
}

function createButtonComponents(parent){
  const cPrim = figma.createComponent(); cPrim.name='Button/variant=primary'; cPrim.resize(120,40);
  const rp = createRoundedRect(0,0,120,40,12,{r:0.43,g:0.36,b:0.94}, null);
  const tp = createTextNode('Primary', 32, 12, 14, {r:1,g:1,b:1}); cPrim.appendChild(rp); cPrim.appendChild(tp);
  const cGhost = figma.createComponent(); cGhost.name='Button/variant=ghost'; cGhost.resize(120,40);
  const rg = createRoundedRect(0,0,120,40,12,{r:0.07,g:0.09,b:0.22},{r:0.16,g:0.18,b:0.33});
  const tg = createTextNode('Ghost', 40, 12, 14, {r:0.91,g:0.91,b:0.96}); cGhost.appendChild(rg); cGhost.appendChild(tg);
  parent.appendChild(cPrim); parent.appendChild(cGhost);
  const set = figma.combineAsVariants([cPrim,cGhost], parent);
  set.name = 'Button'; set.variantGroupProperties = {variant:{values:['primary','ghost']}};
  return set;
}

function createPracticeComponents(parent){
  const cOff = figma.createComponent(); cOff.name='Practice/done=false'; cOff.resize(320,100);
  const rOff = createRoundedRect(0,0,320,100,16,{r:0.27,g:0.24,b:0.56}, {r:0.28,g:0.28,b:0.45});
  const tOff = createTextNode('Практика • 10 мин', 16, 14, 12, {r:0.91,g:0.91,b:0.96});
  cOff.appendChild(rOff); cOff.appendChild(tOff);
  const cOn = figma.createComponent(); cOn.name='Practice/done=true'; cOn.resize(320,100);
  const rOn = createRoundedRect(0,0,320,100,16,{r:0.43,g:0.36,b:0.94}, null);
  const tOn = createTextNode('Готово ✓', 16, 14, 12, {r:1,g:1,b:1}); cOn.appendChild(rOn); cOn.appendChild(tOn);
  // toggle
  cOff.reactions = [{trigger:{type:'ON_CLICK'}, action:{type:'CHANGE_TO', destinationId: cOn.id}}];
  cOn.reactions = [{trigger:{type:'ON_CLICK'}, action:{type:'CHANGE_TO', destinationId: cOff.id}}];
  parent.appendChild(cOff); parent.appendChild(cOn);
  const set = figma.combineAsVariants([cOff,cOn], parent);
  set.name='Practice'; set.variantGroupProperties = {done:{values:['true','false']}};
  return set;
}
...
async function run(){
  ...
  const {container, set: tabSet} = createTabComponents();
  container.x = 1300; container.y = 40;
  const btnSet = createButtonComponents(container);
  const prSet = createPracticeComponents(container);

  frames['Home'].expand();
  figma.notify('ArcheDream high‑fi frames, overlays, and interactive components created');
  figma.closePlugin();
}
